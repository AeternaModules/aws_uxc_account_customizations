output "uxc_account_customizations_id" {
  description = "Map of id values across all uxc_account_customizations, keyed the same as var.uxc_account_customizations"
  value       = { for k, v in aws_uxc_account_customizations.uxc_account_customizations : k => v.id if v.id != null && length(v.id) > 0 }
}
